...
namespace dlib
{
    ...
    std::pair<double, point> letter_box(
    ...
    struct yolo_options
    {
        ...
        {
            DLIB_CASSERT(
                input_size % downsampling_factor == 0,
                "input_size is not a multiple of downsampling_factor");
        ...
        std::pair<matrix<rgb_pixel>, map_type>
            generate_map(const matrix<rgb_pixel>& image, const std::vector<mmod_rect>& boxes)
        {
            ...
            const auto [scale, offset] = letter_box(image, input_size, resized_image);
            for (size_t i = 0; i < boxes.size(); ++i)
            {
                ...
                if (image_rect.contains(oc))
                {
                    ...
                    const auto height = static_cast<float>(bbox.height()) / input_size;
                    // normalized width of the reshaped bounding box
                    const auto width = static_cast<float>(bbox.width()) / input_size;
                    // the row in the ouput map
                    const auto r = oc.y() / downsampling_factor;
                    // the column in the ouput map
                    const auto c = oc.x() / downsampling_factor;
                    // the y offset from the top left corner
                    const auto offset_y = static_cast<float>(oc.y()) / downsampling_factor - r;
                    // the x offset from the top left corner
                    const auto offset_x = static_cast<float>(oc.x()) / downsampling_factor - c;
                    // the class index
                    const size_t idx =
                        std::find(labels.begin(), labels.end(), boxes[i].label) - labels.begin();
                    if (idx < labels.size())
                    {
                        label_map[0](r, c) = 1.f;
                        DLIB_CASSERT(
                            0 <= offset_x && offset_x <= 1,
                            "wrong offset_x: " << offset_x);
                        label_map[1](r, c) = offset_x;
                        DLIB_CASSERT(
                            0 <= offset_y && offset_y <= 1,
                            "wrong offset_y: " << offset_y);
        ...
        matrix<rgb_pixel>
            overlay_map(const matrix<rgb_pixel>& image, const std::vector<mmod_rect>& boxes)
        {
            ...
            for (long r = 0; r < get_grid_size(); ++r)
            {
                for (long c = 0; c < get_grid_size(); ++c)
                {
                    ...
                    {
                        point center(
                            (c + yolo_map[1](r, c)) * downsampling_factor,
                            (r + yolo_map[2](r, c)) * downsampling_factor);
                        ...
                        for (long gr = 0; gr < downsampling_factor; ++gr)
                        {
                            for (long gc = 0; gc < downsampling_factor; ++gc)
                            {
                                ...
                                assign_pixel(
                                    resized_image(
                                        r * downsampling_factor + gr,
                                        c * downsampling_factor + gc),
                                    p);
                            }
                        }
                        // bounding boxes
        ...
        std::pair<double, double> get_row_col_scales(const long nr, const long nc)
        {
            return std::make_pair(
                static_cast<double>(input_size) / nr,
                static_cast<double>(input_size) / nc);
        }
    };
    ...
    void inline deserialize(yolo_options& item, std::istream& in)
    {
        ...
        if (version != 0)
            throw serialization_error(
                "Unexpected version found with deserializing dlib::yolo_options");
        deserialize(item.input_size, in);
        deserialize(item.downsampling_factor, in);
        deserialize(item.labels, in);
        DLIB_CASSERT(
            item.input_size % item.downsampling_factor == 0,
            "input_size is not a multiple of downsampling_factor");
    }

    bool operator<(const mmod_rect& a, const mmod_rect& b)
    {
    ...
    class loss_yolo_
    {
        ...
        template <typename SUB_TYPE, typename label_iterator> void to_label(
            const tensor& input_tensor,
            const SUB_TYPE& sub,
            label_iterator iter,
            const double adjust_threshold = 0) const
        {
            ...
            DLIB_CASSERT(
                input_tensor.num_samples() == output_tensor.num_samples(),
                "num_samples mismatch");
                    ...
                    for (long c = 0; c < output_tensor.nc(); ++c)
                    {
                        ...
                        if (objectness > adjust_threshold)
                        {
                            const float offset_x =
                                out_data[tensor_index(output_tensor, i, 1, r, c)];
                            const float offset_y =
                                out_data[tensor_index(output_tensor, i, 2, r, c)];
                            const float height =
                                std::pow(out_data[tensor_index(output_tensor, i, 3, r, c)], 2);
                            const float width =
                                std::pow(out_data[tensor_index(output_tensor, i, 4, r, c)], 2);
                            float max_value = out_data[tensor_index(output_tensor, i, 5, r, c)];
                            long label_idx = 0;
                            for (long k = 6; k < output_tensor.k(); ++k)
                            {
                                const float value =
                                    out_data[tensor_index(output_tensor, i, k, r, c)];
                                if (value > max_value)
                                {
                                    max_value = value;
                                    label_idx = k - 5;
            ...
            SUBNET& sub) const
        {
            ...
            DLIB_CASSERT(output_tensor.k() >= 6, "YOLO layer requires at least 6 channels");
            DLIB_CASSERT(
                output_tensor.nr() == options.get_grid_size(),
                "wrong output nr: " << output_tensor.nr());
            DLIB_CASSERT(
                output_tensor.nc() == options.get_grid_size(),
                "wrong output nc: " << output_tensor.nc());
            DLIB_CASSERT(
                output_tensor.nr() == grad.nr() && output_tensor.nc() == grad.nc() &&
            ...
            for (long i = 0; i < output_tensor.num_samples(); ++i)
            {
                ...
                DLIB_CASSERT(
                    truth_map_ptr->at(0).nr() == output_tensor.nr() &&
                        truth_map_ptr->at(0).nc() == output_tensor.nc(),
                    "truth size = " << truth_map_ptr->at(0).nr() << " x "
                                    << truth_map_ptr->at(0).nc() << ", output size = "
                                    << output_tensor.nr() << " x " << output_tensor.nc());
            }

            const float* out_data = output_tensor.host();
            float* g = grad.host_write_only();

            // The loss we output is the average loss of over the minibatch and also over each
            // objectness element
            const double scale =
                1.0 / (output_tensor.num_samples() * output_tensor.nr() * output_tensor.nc());
            const double lambda_noobj = 0.5;
            const double lambda_coord = 5.0;

            // extract the subtensors from the output tensor
            resizable_tensor obj_tensor(
                output_tensor.num_samples(),
                1,
                output_tensor.nr(),
                output_tensor.nc());
            resizable_tensor bbr_tensor(
                output_tensor.num_samples(),
                4,
                output_tensor.nr(),
                output_tensor.nc());
            resizable_tensor cls_tensor(
                output_tensor.num_samples(),
                output_tensor.k() - 5,
                output_tensor.nr(),
                output_tensor.nc());
            ...
            resizable_tensor temp_obj_tensor(
                obj_tensor.num_samples(),
                obj_tensor.k(),
                obj_tensor.nr(),
                obj_tensor.nc());
            tt::sigmoid(temp_obj_tensor, obj_tensor);
            resizable_tensor temp_bbr_tensor(
                bbr_tensor.num_samples(),
                bbr_tensor.k(),
                bbr_tensor.nr(),
                bbr_tensor.nc());
            tt::sigmoid(temp_bbr_tensor, bbr_tensor);
            resizable_tensor temp_cls_tensor(
                cls_tensor.num_samples(),
                cls_tensor.k(),
                cls_tensor.nr(),
                cls_tensor.nc());
            ...
            for (long i = 0; i < output_tensor.num_samples(); ++i)
            {
                for (long r = 0; r < output_tensor.nr(); ++r)
                {
                    for (long c = 0; c < output_tensor.nc(); ++c)
                    {
                        ...
                        DLIB_CASSERT(
                            static_cast<long>(y) < cls_tensor.k(),
                            "y: " << y << ", cls_tensor.k(): " << cls_tensor.k());
            ...
            // std::cout << "loss_obj: " << loss_obj;
            // std::cout << ", loss_bbr: " << loss_bbr;
            // std::cout << ", loss_cls: " << loss_cls;
            // std::cout << std::endl;
        ...
        friend void deserialize(loss_yolo_& item, std::istream& in)
        {
            ...
            if (version != "loss_yolo_")
                throw serialization_error(
                    "Unexpected version found when deserializing dlib::loss_yolo_");
        ...
        friend void to_xml(const loss_yolo_ /*item*/, std::ostream& out) { out << "<loss_yolo/>"; }
        ...
        {
            // See:
            // https://github.com/davisking/dlib/blob/4dfeb7e186dd1bf6ac91273509f687293bd4230a/dlib/dnn/tensor_abstract.h#L38
            return ((sample * t.k() + k) * t.nr() + row) * t.nc() + column;
        }
    };

    template <typename SUBNET> using loss_yolo = add_loss_layer<loss_yolo_, SUBNET>;
}  // namespace dlib
